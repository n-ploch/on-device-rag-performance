...
| Step | File | Workflow |
|------|------|---------|
| 1 | [`ragrig-setup.md`](ragrig-setup.md) | Bootstrap venv, start worker (keep the PID of the worker to fetch logs later) |
| 2 | [`ragrig-config.md`](ragrig-config.md) | Interactively create a config YAML (required first step) |
| 3 | [`ragrig-evaluate.md`](ragrig-evaluate.md) | Pre-flight checks, run evaluation via CLI or HTTP API, interpret results. The evaluation run downloads configured models and builds collections if missing. In case of errors, check orchestrator logs first, then grep worker logs |
