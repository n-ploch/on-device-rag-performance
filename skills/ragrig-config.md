...
> "Would you like to use quick defaults and just fill in the essentials, or
> configure every option?"

- **Quick** → follow the [Quick path](#quick-path) below
- **Full** → follow the [Full path](#full-path) below
...
Ask: *"Which generation model do you want to use?
Common choices:
...
Follow-up: *"Quantization level? (`q4_k_m` is a good default, `q8_0` for higher
quality, `q2_k` for minimum RAM)"*
Default: `"q4_k_m"`.

### Q3 — Local or remote generation?

Ask: *"Run the model locally on the worker, or via a remote OpenAI-compatible
API?"*
Default: `"local"`.

If remote: ask for `base_url` (required) and `api_key_env` (default:
`"OPENAI_API_KEY"`).
...
Assemble the YAML using the template below and write to the output path.
Show the user the file content and confirm before writing.
...
run_configs:
  ...
  - run_id: "<Q1_run_id>"
    ...
    generation:
      model: "<Q2_model>"
      quantization: "<Q2_quantization>"
      hosting: "<Q3_hosting>"
      # remote:                 # uncomment when hosting: remote
      #   base_url: "<url>"
...
| Question | Default |
|----------|---------|
| Model (HF repo or API model name) | `"unsloth/Llama-3.2-3B-Instruct-GGUF"` |
| Quantization | `"q4_k_m"` |
| Hosting (`local` / `remote`) | `"local"` |

If `hosting: remote`, also ask:
- `base_url` (**required**)
- `api_key_env` (default: `"OPENAI_API_KEY"`)
- `rate_limit_rps` (default: `null`)
